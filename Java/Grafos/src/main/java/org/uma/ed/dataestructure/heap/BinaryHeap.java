...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T extends Comparable<? super T>> BinaryHeap<T> empty() {
    return new BinaryHeap<T>(Comparator.naturalOrder());
  }
  ...
  public static <T> BinaryHeap<T> of(Comparator<T> comparator, T... elements) {
    return new BinaryHeap<>(comparator, DEFAULT_INITIAL_CAPACITY, elements);
  }
  ...
  public static <T> BinaryHeap<T> from(Comparator<T> comparator, Iterable<T> iterable) {
    ...
    BinaryHeap<T> res = new BinaryHeap<>(comparator, size);
    for (T var : iterable){
      res.insert(var);
    }
    return res;
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
    return new BinaryHeap<>(that.comparator, that.size(), that.elements);
  }
  ...
  private void heapifyUp(int index) {
    if (!isRoot(index)) {
      ...
      while (!isRoot(index) && lessThan(index, parent)) {
        swap(index, parent);
        index = parent;
        parent = parent(index);
      }
  ...
  private void heapifyDown(int index) {
    ...
    int minChild;
    boolean stop = false;
    while (!isLeaf(index) && !stop){
      left = hasLeftChild(index)? leftChild(index) : -1;
      right = hasRightChild(index)? rightChild(index) : -1;
      if (left != -1 && right != -1){
        minChild = lessThan(left, right)? left : right;
      }else{
        minChild = left;
      }
      if (lessThan(index, minChild)){
        stop = true;
      }else {
        swap(index, minChild);
        index = minChild;
