...
public class ArrayQueue<T> extends AbstractQueue<T> implements Queue<T> {
  ...
  public static <T> ArrayQueue<T> of(T... elements) {
    ArrayQueue<T> newQueue = new ArrayQueue<>(elements.length);
    for (var el: elements){
      newQueue.enqueue(el);
    }
    return newQueue;
  ...
  public static <T> ArrayQueue<T> copyOf(Queue<T> that) {
    ArrayQueue<T> newQueue = new ArrayQueue<>(that.size());
    while(!that.isEmpty()){
      newQueue.enqueue(that.first());
      that.dequeue();
    }
    for (var el: newQueue.elements){
      that.enqueue(el);
    }
    return newQueue;
  }
  ...
  public void enqueue(T element) {
    ensureCapacity();
    this.last = this.advance(this.last);
